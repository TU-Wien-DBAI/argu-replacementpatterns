...
def convertCore(outer,inner):
	coreString = "core(["

	for arg in list(string.ascii_lowercase)[outer:outer+inner]:
		coreString += arg + ","
...
def convertList(coreAmount,afSize,complete):
	domainStrings = convertDomain(coreAmount+afSize)
	coreString = convertCore(afSize,coreAmount)
...
def convertResultToLog(stringList,afSize,coreAmount,facts,afno,enc,rest):
	if len(stringList) != 3:
	...
	log = log + "\ncore:;" + convertCore(afSize,coreAmount)[6:-3].replace(',','') + "\n"
	...
	for fact in factList:
		...
		if(restrictLog(dataList[af1no],dataList[af2no],rest)):
			log = log + dataList[af1no] + ";"	
			log = log + dataList[af2no] + "\n"
			
	return log
	
def restrictLog(af1data,af2data,rest):
	if not (rest is 1 or rest is 2):
		print rest
		return True
	else:
		af1split = af1data.split(';')
		af2split = af2data.split(';')
		
		if af1split[1] == af2split[1]:
			if rest is 2:
				return False
			elif rest is 1 and af1split[2] == af2split[2]:
				return False
